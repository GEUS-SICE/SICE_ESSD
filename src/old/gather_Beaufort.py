...
for year in years:
    ...
    dates=datesx(date(int(year), 4,21),date(int(year), 4, 27))
    # dates=datesx(date(int(year), 7,14),date(int(year), 7, 15))
    ...
    # bands=['r_TOA_02','r_TOA_10','r_TOA_11','r_TOA_21','albedo_bb_planar_sw']
    ...
    output_path=output_base_path + os.sep + region + os.sep + year + os.sep
    # output_path=output_base_path + os.sep
    output_path=f'/Users/jason/0_dat/S3/opendap/{region}_1000m/'+ year + os.sep
    os.system('mkdir -p '+output_path)
    
    # loop over dates
    for d in dates:
        # DATASET_ID = 'sice_500_' + + '.nc'
        # =
        print(d)
        ...
        for var in bands:
            ...
            datex=d.replace('-', '_')
            try:
                ds = xr.open_dataset(f'https://thredds.geus.dk/thredds/dodsC/SICE_1000m/Beaufort/SICEv2.3.2_Beaufort_1000m_{datex}.nc')
                print(var)
