...
@author: Adrien Wehrlé, EO-IO, 2021
...
# TODO: add resample option by computing the resolutions of both grids
# 1) check the resolution of reference and match grids:
#    - if match_res > ref_res: resample match grid
#    - if match_res < ref_res: sample several times the same point (nothing needed)
@timer
def match_m2m(
    ...
    reference_grid_resolution = np.nanmean(np.diff(reference_grid[0, :, 0]))
    match_grid_resolution = np.nanmean(np.diff(match_grid[0, :, 0]))

    if downsample and (int(reference_grid_resolution) > int(match_grid_resolution)):
        print(
            "Reference resolution is higher than match resolution, no downsampling needed"
        )

    if downsample or int(reference_grid_resolution) < int(match_grid_resolution):
        ...
        # stack to 3D
        m2m_results = np.dstack((reference_grid[:, :, 2], tD_resampled_values))
...
@timer
def match_gt2gt(reference_gtiff_file: str, match_gtiff_file: str) -> np.ndarray:
    """Data matching between two GeoTIFF files using Rasterio, mapping
    match_gtiff_file to reference_gtiff_file.

    :param reference_gtiff_file: Path to the GeoTIFF file to be used
      as reference.
    :type reference_gtiff_file: str

    :param match_gtiff_file: Path to the GeoTIFF file to be matched to
      reference_gtiff_file.
    :type match_gtiff_file: str

    :return: A 3D array of two superimposed 2D arrays containing the
      pixels values associated to reference_gtiff_file and the
      matching pixel values of match_gtiff_file.
    :rtype: numpy.ndarray
