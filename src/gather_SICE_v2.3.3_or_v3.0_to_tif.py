...
for year in years:
    ...
    # dates=datesx(date(int(year), 7,14),date(int(year), 7, 14))
    # dates=datesx(date(int(year), 7,8),date(int(year), 7, 8))
    # dates=datesx(date(int(year), 7,30),date(int(year), 7, 31))
    ...
    output_path='/Users/jason/0_dat/S3/opendap/Greenland_all_pixels/'+ year + os.sep
    output_path=f'/Users/jason/0_dat/S3/opendap/Greenland_{resolution[version_index]}m/'
    os.system('mkdir -p '+output_path)
    output_path=f'/Users/jason/0_dat/S3/opendap/Greenland_{resolution[version_index]}m/'+ year + os.sep
    os.system('mkdir -p '+output_path)

    # loop over dates
    for d in dates:
        if version_index==0:
            DATASET_ID = 'SICEv2.3.2_Greenland_1000m_' + d.replace('-', '_') + '.nc'
        else:
        ...
        for var in bands:
            ...
            if not(test_file.is_file()):
                fn=f'https://thredds.geus.dk/thredds/dodsC/SICE_{resolution[version_index]}m/Greenland/{DATASET_ID}'
                print(f'gathering {var} from {d}')
                # fn='/Users/jason/0_dat/S3/opendap/Greenland_1000m/SICEv2.3.2_Greenland_1000m_2017_05_01.nc'
                try:
